...
export class RegisterPersonValidator {
  ...
  public async validate(registerPerson: RegisterPerson,): Promise<AppNotification> {
    ...
    const telefono: string = registerPerson.telefono ? registerPerson.telefono.trim() : '';
    if (telefono.length <= 0) {
      notification.addError('Se requiere de un telefono', null);
    }

    const edad: string = registerPerson.edad ? registerPerson.edad.trim() : '';
    if (edad.length <= 0) {
      notification.addError('Se requiere de la edad', null);
    }

    const codeValidation: string = registerPerson.codeValidation ? registerPerson.codeValidation.trim() : '';
    if (codeValidation.length <= 0) {
      notification.addError('Se requiere de un codigo de verification', null);
    }
