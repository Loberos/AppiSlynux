...
import { CodeVarificationValue } from 'src/register/infrastructure/persistence/values/codeverification.value';
import { EdadValue } from 'src/register/infrastructure/persistence/values/edad.value';
import { TelefonoValue } from 'src/register/infrastructure/persistence/values/telefono.value';
...
export class PersonMapper {
  public static dtoRequestToCommand(registerPersonRequest: RegisterPersonRequest) {
    return new RegisterPerson(
      ...
      registerPersonRequest.edad,
      registerPersonRequest.telefono,
      registerPersonRequest.correo,
      registerPersonRequest.contraseña,
      registerPersonRequest.codeValidation,
    );
  }
  ...
  public static domainToDtoResponse(person: Person) {
    return new RegisterPersonResponse(
      ...
      person.getEdad().getTedad(),
      person.getTelefono().getTelefono(),
      ...
      person.getCodeValidation().getCodeValidation()
    );
  }
  ...
  public static commandToDomain(command: RegisterPerson, userId: number): Person {
    ...
    const edad: PersonEdad = PersonEdad.create(command.edad);
    const telefono: PersonTelefono = PersonTelefono.create(command.telefono);
    ...
    const codeValidation: CodeValidationPerson = CodeValidationPerson.create(command.codeValidation);
    const auditTrail: AuditTrail = AuditTrail.from(DateTime.utcNow(),UserId.of(userId),null,null);
    const clientId: ClientId = ClientId.of(userId);

    let person: Person = PersonFactory.from(clientId,personName,dni,edad,telefono,auditTrail,correo, contraseña, codeValidation);
    return person;
  }
  ...
  public static domainToEntity(person: Person): PersonEntity {
    ...
    personEntity.edad = EdadValue.from(person.getEdad().getTedad());
    personEntity.telefono = TelefonoValue.from(person.getTelefono().getTelefono());
    ...
    personEntity.codigoVerification = CodeVarificationValue.from(person.getCodeValidation().getCodeValidation());
  ...
  public static entityToDomain(personEntity: PersonEntity): Person {
    ...
    const edad: PersonEdad = PersonEdad.create(personEntity.edad.value);
    const telefono: PersonTelefono = PersonTelefono.create(personEntity.telefono.value);
    ...
    const codeValidation: CodeValidationPerson = CodeValidationPerson.create(personEntity.codigoVerification.value);
    ...
    let person: Person = PersonFactory.withId(clientId, personName, dni,edad,telefono, auditTrail,correo,contraseña,codeValidation);
    return person;
  }
  ...
  public static ormToPersonClientDto(row: any): PersonClientDto {
    ...
    dto.codeValidation = row.codeValidation;
    return dto;
  }
}